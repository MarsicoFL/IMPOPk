...
///
/// ```rust
/// use hprc_ibd::{IbdError, Region};
///
/// let result = Region::parse("invalid", None);
/// match result {
///     Err(IbdError::InvalidParameter(msg)) => {
...
/// Convenience type alias for Results with [`IbdError`].
...
impl Region {
    ...
    /// # Errors
    ///
    /// Returns [`IbdError::InvalidRegion`] if the format is malformed.
    /// Returns [`IbdError::InvalidParameter`] if chromosome-only format is used without length.
...
mod tests {
    ...
    fn test_region_parse_missing_dash() {
        ...
        match result {
            Err(IbdError::InvalidRegion(s)) => assert_eq!(s, "chr1:1000"),
    ...
    fn test_region_parse_non_numeric_start() {
        ...
        assert!(result.is_err());
        match result {
            Err(IbdError::InvalidRegion(s)) => assert_eq!(s, "chr1:abc-2000"),
            _ => panic!("Expected InvalidRegion error"),
        }
    }
    ...
    fn test_region_parse_non_numeric_end() {
        ...
        assert!(result.is_err());
        match result {
            Err(IbdError::InvalidRegion(s)) => assert_eq!(s, "chr1:1000-xyz"),
            _ => panic!("Expected InvalidRegion error"),
        }
    }
    ...
    fn test_region_parse_chrom_only_without_length() {
        ...
        assert!(result.is_err());
        match result {
            Err(IbdError::InvalidParameter(s)) => {
